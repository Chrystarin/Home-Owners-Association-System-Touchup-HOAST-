...
const { getRoles } = asyncHandler({
    getRoles: async (req, res, next) => {
        ...
        const residents = await Home.find({
        ...
        const roles = {
            ...
            // homeowner: homes.map(({ homeId, paidUntil }) => ({
            // 	homeId,
            // 	isUnpaid: paidUntil < current
            // })),
            homeowner: homes.map(({ homeId }) => homeId),
            resident: residents.map(({ homeId }) => homeId),
            delinquent:
                homes.filter(({ paidUntil }) => paidUntil < current).map(({ homeId }) => homeId) +
                residents.filter(({ paidUntil }) => paidUntil < current).map(({ homeId }) => homeId)
        };
