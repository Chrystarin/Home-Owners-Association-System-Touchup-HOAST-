...
const { UserNotFoundError, NotFoundError, ForbiddenError, ResidentNotFoundError } = require('../helpers/errors');
...
const getResidents = async (req, res, next) => {
    ...
    if (residentId) {
        residents = residents.find(({ user: { userId } }) => residentId == userId);
...
const addResident = async (req, res, next) => {
    const { firstName, lastName, email, residentType, title } = req.body;
    const { home } = req.user;
    ...
    const residentExists = home.residents.find(({ user: { _id } }) => _id.equals(resident._id));
    ...
    home.residents.push({ user: resident._id, type: residentType, title: title });
    await home.save();

    res.status(201).json({
        message: 'Resident added',
        resident,
        residentPassword: genPass
    });
};
...
const removeResident = async (req, res, next) => {
    ...
    if (residentId === user.userId) throw new ForbiddenError('Owner can not be inactive');

    // Find user index
    const resident = home.residents.find(({ user: { userId }, status }) => userId === residentId && status === 'active');
